...
- [DevSwarm layered recovery — full history](#devswarm-layered-recovery--full-history)
...
| `auto-handover.js` | UserPromptSubmit | **Auto-handover trigger, ON by default.** When the MAIN agent's ESTIMATED context usage (from the transcript's own recorded token usage, `hooks/lib/context-pct.js` — not the harness's own figure) first crosses the threshold (default 85%, `autoHandover` section of `~/.anti-hall/settings.json` via `hooks/lib/settings.js`, or env `ANTIHALL_AUTO_HANDOVER_PCT`, `0` = off), tells the agent to — without asking the user first — self-write an anti-hall handover (never delegated; the `/anti-hall:handover` skill's own self-write mandate), tell the user and list the saved paths, and urge `/compact`/`/clear`. Fires once per session at the crossing; re-arms once usage drops back below threshold (a compact/clear, in practice). Also sends a short milestone nag every further `nagStepPct` points of growth (default 5) unless `nag:false`. Never fires for a subagent turn (`coordinator-detect.js`). Configure via the `auto-handover-config` skill / `scripts/auto-handover-config.js`. Skip-guard hatch: `auto-handover`. Shared verbatim with the Codex port. |
| `auto-handover-pause-nag.js` | Stop | The auto-handover trigger's companion: once it has fired this session and usage is still over threshold, sends one short reminder at a genuinely quiet point — no open TodoWrite work, no subagent spawned in the last 2 minutes — at most once per `nagQuietMin` minutes (default 15). Uses the shared `stop-policy.js` `stop_hook_active` check so its own block-with-reason (the only non-blocking-adjacent way a Stop hook can surface text in this harness) is never re-triggered by its own answer. Silent when `autoHandover.nag` is false or the feature is disabled. Shared verbatim with the Codex port. |
| `hooks/lib/settings.js` | Shared module (not a hook) | Sectioned, persisted plugin settings at `~/.anti-hall/settings.json` (`{ "<section>": { "<key>": value } }`) — `load()`/`get()`/`set()`/`path()`, atomic writes, env-override support per key. `auto-handover-config.js` is its first consumer (section `autoHandover`); a future settings surface reuses the same file/API. |
