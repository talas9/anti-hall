...
| `companion/mcp-reaper.js` (+ `install-reaper.js`) | Interval companion (not a hook) | **OPT-IN**, macOS + Linux. Kills ONLY orphaned MCP-server processes (parent already died); also, separately (`guards.reaperCodexBroker`, default on), abandoned openai-codex plugin `app-server-broker.mjs` helper processes — spawned detached on purpose, so PPID 1 is normal for a LIVE one; reaped only when its `--cwd` is gone or no live claude/codex process has a cwd equal to/an ancestor of/a descendant of it, and it is past `guards.reaperCodexBrokerMinAgeS` (30min default). Install via `node companion/install-reaper.js` (`--uninstall` to remove); Windows is a documented no-op. See [`plugins/anti-hall/companion/README.md`](../plugins/anti-hall/companion/README.md). |
...
| `guards.reaperCodexBroker` adv | `true` | `ANTIHALL_REAPER_CODEX_BROKER` | companion/mcp-reaper.js: also reap abandoned openai-codex plugin `app-server-broker.mjs` helper processes — reaped only via proof of abandonment (--cwd gone, or no live claude/codex process's cwd is equal to/an ancestor of/a descendant of it — a session at a workspace root still owns a broker `--cwd` inside a submodule under it), never by PPID (spawned detached on purpose). |
| `guards.reaperCodexBrokerMinAgeS` adv | `1800` [0..] | `ANTIHALL_REAPER_CODEX_BROKER_MIN_AGE_S` | Minimum age (seconds) before an abandoned-looking `app-server-broker.mjs` is eligible for the reaper above (30min default; conservative since PPID gives no signal). |
...
| `jevIntegrations.postHandoverGate` | `shadow` (on/shadow/off) | — | Does this new request fit in the remaining post-handover context budget (advisory trust; askDetached fire-and-forget; shadow-only — never changes the gate directive). |
