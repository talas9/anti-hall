...
| `autoHandover.maxTokens` | `170000` [0..] | `ANTIHALL_AUTO_HANDOVER_MAX_TOKENS` | Absolute context-token ceiling that also triggers the handover, whichever of pct/maxTokens comes first (170000 = 85% of a 200k window, so a 1M session hands over at ~170k); 0 = no ceiling. |
