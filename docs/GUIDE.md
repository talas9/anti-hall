...
| `guards.reaperCodexBroker` adv | `true` | `ANTIHALL_REAPER_CODEX_BROKER` | companion/mcp-reaper.js: also reap abandoned openai-codex plugin `app-server-broker.mjs` helper processes — reaped only via proof of abandonment (--cwd gone, or no live claude/codex process owns that cwd), never by PPID (spawned detached on purpose). |
