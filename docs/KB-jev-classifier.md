...
| `postHandoverGate` | auto-handover.js (`UserPromptSubmit`, only while the post-handover new-work gate is armed; Claude + Codex, shared file) | `advisory` (does this request fit in the remaining post-handover budget; baseline always `null`) | `askDetached` (critical path, zero latency) | `shadow` — never changes the gate directive; the size judgment stays the agent's own | none wired |
