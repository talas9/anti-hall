...
- `PostToolUse` runs after success — **cannot block**, but can replace output / add audit context. Use for formatting, linting, audit logs ([pixelmojo](https://www.pixelmojo.io/blogs/claude-code-hooks-production-quality-ci-cd-patterns)). [UPDATE 2026-08-22: "add audit context" confirmed empirically for Claude Code on CLI 2.1.238 — see the `[CORRECTION 2026-08-22]` annotation at §1.4 (`:47`). "Cannot block" still stands.]
...
- **Output cap ~10,000 chars, PER HOOK COMMAND** — the failure mode is NOT clean truncation, it is **spill-to-file**: a payload `<=10,000` chars lands 100% inline; a payload `>=10,001` delivers only the first ~2,000 chars inline plus a file path the model must *choose* to open, so the tail effectively never reaches context. Measured boundary (this repo): `<=10,000` -> 100% inline, `>=10,001` -> ~2,000 inline. Because the cap is per hook COMMAND, splitting an over-cap injection across two hook commands (each `<10,000`) lands both fully inline — the lever `verify-first-full.js` + `verify-first-orch.js` use ([official ref](https://code.claude.com/docs/en/hooks)).
