...
|---|---|---|
| **Version** | `0.44.0` | `plugin.json` `version` — the single authority. Marketplace entry carries NO `version` (avoids the silent-precedence trap). Note: this row is kept current at each release, but the surrounding hook enumeration below still reflects 0.39.0-era ground truth (fable-availability.js/progress-prune.js/session-history-index.js missing) — tracked separately, not backfilled here. |
