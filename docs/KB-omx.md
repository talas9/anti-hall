...
| Model routing | OMX installed `ultrawork` role-routing table (`skills/ultrawork/references/agent-tiers.md`) maps roles to abstract tiers (`LOW`/`STANDARD`/`THOROUGH`) and postures (`frontier-orchestrator`/`deep-worker`/`fast-lane`), not to concrete Codex model IDs. Actual `model`/`model_reasoning_effort` resolution defaults to session `.codex/config` (or `$team` env-var defaults), except named pins — e.g. ralplan pins `planner`/`architect` to exact `gpt-5.4-mini`. `gpt-5.4-mini` is kept as the cheap-default pin (still cheapest confirmed-priced tier); `gpt-5.6-luna` (GA 2026-07-09, see `KB-gpt-5.6.md`) is a selective alternative where its capabilities suit the task. | Anti-hall must keep debate gates on `gpt-5.6-sol` (migrated from `gpt-5.5`, see `KB-sonnet-5.md` §8) and avoid all-frontier fan-out; do not assume OMX auto-binds a concrete model per role — see `KB-sonnet-5.md` §8 for the "no auto-select" framing this reconciles with. |
