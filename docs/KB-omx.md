...
| Model routing | OMX installed `ultrawork` role-routing table (`skills/ultrawork/references/agent-tiers.md`) maps roles to abstract tiers (`LOW`/`STANDARD`/`THOROUGH`) and postures (`frontier-orchestrator`/`deep-worker`/`fast-lane`), not to concrete Codex model IDs. Actual `model`/`model_reasoning_effort` resolution defaults to session `.codex/config` (or `$team` env-var defaults), except named pins — e.g. ralplan pins `planner`/`architect` to a literal model slug (OMX's own file, outside anti-hall's control). **v0.103.0:** anti-hall itself no longer pins a literal slug anywhere — `gpt-5.4`/`gpt-5.4-mini`/`gpt-5.3-codex(-spark)` were removed from Codex's catalog and started returning 400; anti-hall now resolves **frontier**/**workhorse**/**fast** by category from Codex's own live model cache (`plugins/anti-hall/companion/lib/codex-models.js`), never a hardcoded name. | Anti-hall must keep debate gates on the resolved **frontier** category (see `KB-sonnet-5.md` §8) and avoid all-frontier fan-out; do not assume OMX auto-binds a concrete model per role — see `KB-sonnet-5.md` §8 for the "no auto-select" framing this reconciles with. |
...
- `anti-hall-ship-it` (Codex-native; replaced `anti-hall-feature-launch` 2026-07-05) should say Codex feature work can use OMX `$plan`, `$ralplan`, `$team`, `$ralph`, `$ultragoal`, or `$pipeline` after the planning gates.
