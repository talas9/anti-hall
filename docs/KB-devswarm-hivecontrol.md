...
| **auto-archive gate (h), no re-archive at the same HEAD (0.108.3 fix)** | The owner's only undo for an auto-archive is unarchiving the workspace in the DevSwarm app (there is no CLI unarchive). Its `done` gate at that HEAD stayed set, so the sweep used to re-archive it once the focus and idle windows passed. Each successful auto-archive now records `{id, doneHead, at}` (plus `ts`) in `~/.anti-hall/logs/devswarm-auto-archive.ndjson`, and `evaluateCandidate` blocks with `h-rearchive` while a successful record for any of the candidate's ids matches its current HEAD. A new `done` at a new HEAD makes it eligible again. The log is append-only: no retire/restore, rotation or sweep path touches it. Records written before 0.108.3 carry no `doneHead` and never match. | `autoArchivedAt`/`evaluateCandidate`/`autoArchiveSweep` `companion/lib/devswarm-lifecycle.js` |
