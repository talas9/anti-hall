...
>
> **Resolved by source inspection (DevSwarm 2.5.3 bundled CLI, `workspace create` action, 2026-09-25):**
> `sourceBranch` = `-s/--source` if given; else, inside DevSwarm (`DEVSWARM_REPO_ID` set), the
> caller's CURRENT branch (`getCurrentBranch()`, falling back to the builder's own branch); else
> `DEVSWARM_DEFAULT_BRANCH || DEVSWARM_SOURCE_BRANCH || "main"`. It is sent to the app as a branch
> NAME (`apiClient.createWorkspace({ sourceBranch, ... })`) and becomes the child's recorded parent, so
> the child forks from the LOCAL branch of that name — a stale local `main` yields a stale child.
> anti-hall's `devswarm.js spawn` fast-forwards that local branch to `origin/<default>` first (0.108.5).
...
| `archive <id>` | Archives anti-hall's OWN registry state: moves the descriptor into `archived/` (renames — never unlinks) and tombstones the store registry entry. **v0.108.4 fix**: DevSwarm >= 2.5.3 DOES have a real `workspace archive <id>` hivecontrol verb (a 0.108.3-era claim that it had "no teardown command" was false — proven live 2026-09-25: it sets isActive=0/isHidden=1). When the capability gate (`companion/lib/devswarm-capabilities.js` `can('workspace.archive')`) allows it, `cmdArchive` now ALSO archives the workspace in the app, EXPLICIT id always (never the "current workspace" default), retrying once on the known-flaky "Could not confirm terminal process boundary" error. The result carries `appArchive: {attempted, ok, retried?, error?}`; `manualStep` is now accurate either way (never the old false claim) — it confirms the app-side archive when it succeeded, or names the real reason (dormant capability / a genuine app-side failure) and points at running `hivecontrol workspace archive <id>` by hand otherwise. | `cmdArchive`/`attemptAppArchive` |
...
| `done [<id>] [--summary TEXT]` | **CHILD-side (0.108.3).** The structured done-report. Sets the `done` gate on the caller's OWN workspace id (resolved from cwd via the sender-identity resolver `send` uses; an explicit `<id>` must match, else `reason:'not-own-workspace'`) through the `gate` path, then sends the Primary ONE `[[ANTIHALL_DONE]]`-prefixed mesh-direct message (`kind:'done'` in the result; `DONE_REPORT_MARKER` in `devswarm-store.js`). The message hash is `done:<id>:<HEAD>`, so a re-run on the same commit inserts nothing (`duplicate:true`). The gate row records the worktree HEAD (`set_by` `devswarm-done@<sha>`, projected as `doneHead`); auto-archive gate (a) honours it only while that sha is still HEAD. Never sets `merged`/`tests_passed` — auto-archive gate (b) proves the merge with `gitMergeProof` (HEAD an ancestor of the remote default branch — the same check `gate --set merged` records as `merged_verified`); only when git can't decide does a `merged` gate verified at the current HEAD, or the app's merged PR row, count. A git "not an ancestor" (unmerged commits, or a squash merge) always blocks. Refused from the Primary checkout (`reason:'primary-checkout'`). Fail-open on the message leg: the gate row is what auto-archive gate (a) reads. `roster` shows a child whose `done` gate is set with the `done` + `archive-pending` hints. The child's SessionStart directive (`hooks/devswarm-child-role.js`, both agents) tells it to run this once its work is merged. | `cmdDone` `devswarm.js`, dispatch `case 'done'` |
| **auto-archive gate (d), DIRECT-only (0.108.3 fix)** | `computeSummary` splits a workspace's unread into `unread`/`directUnread` (this partition's own direct rows) and `broadcastUnread` (the shared mesh partition's non-heartbeat rows past this workspace's `broadcastCursorValue`) — two structurally distinct partitions, never one count. Pre-0.108.3, `devswarm-lifecycle.js`'s `unreadFact` summed BOTH into gate (d)'s `toChild`, so a finished lane with a broadcast/FYI backlog (e.g. `d-unread (to=197 from=0)`, all 197 from the shared broadcast partition) could never auto-archive. `unreadFact(..., { excludeBroadcast: true })` now makes gate (d) DIRECT-only — it blocks only on unread DIRECT rows addressed to the child, or unread rows FROM the child in another partition the Primary hasn't seen (its done report, via the unchanged `unreadFrom` probe) — never on broadcast/mesh-wide rows. The plan's blocker detail now reads `to_direct=<n> to_broadcast=<n> from=<n>` so the owner sees the split. Scoped to gate (d) alone: `evaluateArchived` (the separate `prune-archived` gate) still calls `unreadFact` without `excludeBroadcast` and is unchanged. | `unreadFact`/`evaluateCandidate` `companion/lib/devswarm-lifecycle.js`, `unread`/`broadcastUnread` `companion/lib/devswarm-store.js` |
| **auto-archive gate (h), no re-archive at the same HEAD (0.108.3, durable state 0.108.4)** | The owner's only undo for an auto-archive is unarchiving the workspace in the DevSwarm app (there is no CLI unarchive). Its `done` gate at that HEAD stayed set, so the sweep used to re-archive it once the focus and idle windows passed. Each successful auto-archive records `{id, doneHead, at}` (plus `ts`) in TWO places: the existing `~/.anti-hall/logs/devswarm-auto-archive.ndjson` (unchanged, still written every time), and — **v0.108.4** — a purpose-built, never-rotated/pruned durable file `~/.anti-hall/devswarm/auto-archived.json` (shape `{id: [{doneHead, at}, ...]}`). `autoArchivedAt` checks the durable file FIRST, falling back to the ndjson log only for a record the migration below has not backfilled — the durable file exists because the ndjson log is a general-purpose LOG this codebase's other log-rotation/pruning paths could legitimately remove, which would silently re-open the exact hole gate (h) exists to close. `evaluateCandidate` blocks with `h-rearchive` while either source has a successful record for any of the candidate's ids matching its current HEAD; a new `done` at a new HEAD makes it eligible again. Both sources are append-only: no retire/restore, rotation or sweep path touches either. An idempotent forward-migration (`migrateAutoArchivedState`, wired into both `doctor --repair`'s `migrate-auto-archived-state` and `update.js`'s post-pull `auto-archived-state-migrate` stage — same AUTO-SAFE posture as the other state migrations) seeds the durable file from any pre-0.108.4 ndjson records; running it is a courtesy, not a correctness prerequisite, since the ndjson fallback keeps gate (h) working either way. Records written before 0.108.3 carry no `doneHead` and never match either source. | `autoArchivedAt`/`autoArchivedStateAppend`/`migrateAutoArchivedState`/`evaluateCandidate`/`autoArchiveSweep` `companion/lib/devswarm-lifecycle.js` |
...
| `reconcile-active [--active id,...] [--allow-empty] [--stdin] [--yes\|--confirm]` | **v0.62.0, NEW; v0.107.1: app-DB-gated.** Project-scoped. A current workspace NOT in the set is archived ONLY when the DevSwarm app database (`builders.isActive=0 AND isHidden=1`) proves it archived. Otherwise it is kept (`keptNotArchivedInApp`), and an unreadable app DB archives nothing. Previously: archives every current (non-archived) workspace of THIS project NOT in `--active` (ids also accepted via `--stdin`, newline/space/comma-separated); a match always SPARES a workspace — the safe direction, an active one is never archived. Ids match by full id or a >=4-char prefix / >=8-char embedded-hex substring (roster-display-friendly). Refuses an empty active set unless `--allow-empty` is passed explicitly. Dry-run by default; `--yes`/`--confirm` applies. | `cmdReconcileActive` `devswarm.js:3118`, dispatch `devswarm.js:3440` |
...
- `roster` — `instances` + hint `instance-split`, and `phantom` (defects d3d571495bf6 / 298b79969409): `instances` (item b) is the PEAK number of DISTINCT `instanceNonce` values seen ACTIVE AT THE SAME TIME on a row's own outbound broadcast/heartbeat rows within the shared liveness freshness window (`DEFAULT_HEARTBEAT_FRESH_MS`, the SAME window `hasFreshHeartbeat` uses); present ONLY when at least one instanceNonce was seen (a row with none carries no `instances` key at all — byte-identical to the pre-fix shape). "Same time" is decided by `computeInstanceNonceCounts`'s concurrency sweep, not raw freshness: each nonce's own `[min,max]` activity span is padded by `INSTANCE_SPLIT_CONCURRENT_GAP_MS` (60s) and only nonces whose padded spans overlap count toward the peak — this is the fix for a field-confirmed false positive (2026-09-25): `deriveInstanceNonce` legitimately changes on every process restart, so a plain "both nonces have a row somewhere in the last 15 minutes" test flagged every ordinary restart as a split (the dying process's last heartbeat was still <15min old when the fresh process's first heartbeat landed), even though the two processes never coexisted — confirmed against real `~/.anti-hall` data, where every observed nonce transition was sequential with a multi-minute gap. `> 1` additionally pushes the `instance-split` hint, meaning two live OS processes were both sending mesh traffic under one sessionId/row identity AT THE SAME TIME. `phantom` is the unrelated pre-existing hint: it fires when a row would otherwise carry no dormancy hint at all (reading as active) but the shared `computeRowLive` predicate — the SAME one `diagnose`'s `live` field now uses — finds no real sessionId and no fresh heartbeat, closing a case where a child trusted an empty roster hint and stranded mail on a row `diagnose` already reported as `live:false`. Neither hint is ever applied to native (`source:'native'`) children.
...
**wake-watch (`devswarm-wake-watch.js`) REFUSED TO ARM diagnostics (defect 8143ced316d3; extended v0.102.2):** the refusal line now names the existing holder's `pid`, `age`, plugin `version`, and — as of v0.102.2 — its owning `sessionId` and absolute acquire time, so a reader can tell a LIVE holder from a stale lock without reaching for `ps`. A peer session read a bare `lock-held` as a stale lock and filed it as a defect before checking; the lock was live and correctly held. The `sessionId`/`ts` fields are ADDITIVE to the lock JSON: a lock written by a pre-v0.102.2 watcher parses with them absent and degrades to 'unavailable', never a crash — which matters because long-lived watchers pin the plugin version that armed them and older ones stay resident for days. Original fields: `pid`, `age`, `version` (e.g. `holder pid=1234 age=185s version=0.97.1`) instead of a bare "already watched" message. A stale-but-still-alive holder (its lock's `ts` has gone stale because it stopped calling `release.restamp()` on its poll ticks — a HUNG watcher, not an exited one) can now be taken over: the lock's steal-check opts in via `allowStaleLiveSteal` (default `false`/opt-in for every other lock caller — a one-shot `inbox pull` drain lock never takes this path). A watcher that loses its lock this way exits on its next tick once its `restamp()`/`release()` call finds the token no longer matches.

**Submodule-aware identity resolution (`resolveWorktreeNoSpawn`, v0.102.2):** the parent gate
derived the Primary's own id from a pure-fs walk (`findGitToplevel`) while the family grouping
key used `canonicalMeshId` -> `resolveCallerWorktree` (git rev-parse). **Inside a git submodule
those two disagree**, because a submodule's `.git` is a FILE, not a directory — the fs walk's
`statSync` succeeds and it stops at the submodule instead of reaching the superproject. The gate
then minted an own id that was a bare path hash of the submodule directory (no descriptor, no
registry row; the CLI reported `known:false`), synthesised a self-row for it with no
...
git at all — a submodule's gitdir contains a `.git/modules/<name>` segment, a linked worktree's
contains `.git/worktrees/<name>` — and simply does not stop walking at a submodule boundary,
since a submodule always sits inside its superproject's working tree. The gate tries it first and
falls back to the spawning `resolveCallerWorktree` only if it returns null. Measured cold, in a
fresh process (which is how a hook always runs): **3.0 ms** versus **380 ms** for the spawning
path, so the correctness fix also removed a ~127x cost and dropped the 15,700-line
`scripts/devswarm.js` require from the common hot path.
