...
| `healthcheck [--json]` | **v0.61.0, NEW.** Scriptable pass/fail gate over the SAME data `diagnose` computes (one shared `computeDiagnosis`, two presentations). `degraded` iff `orphans>0 \|\| stale>0 \|\| splits>0` (phantoms/unreadTotal are reported but never gate). No `--json` prints one compact human line (e.g. `healthcheck: ok [orphans=0 stale=0 splits=0 phantoms=0 unread=0]`); exit `0`=healthy, non-zero=degraded, for monitors/CI/the ingest daemon. | `cmdHealthcheck` `devswarm.js:2818`, `healthcheckHumanLine` `devswarm.js:2848`, dispatch `devswarm.js:3418` |
