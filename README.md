...
>
> **Debate roster (TRIO):** Reviewer = Sonnet 5 (`model:"sonnet"`, effort `xhigh`); Auditor = latest Claude Opus (`model:"opus"`, divergent regression/coupling lens, effort `high`); Critic = latest OpenAI Codex at max reasoning (Opus adversarial-persona fallback when Codex unavailable). All three dispatched in the same message for true parallelism. Model floor for fallback seats = Opus; never a cheaper model. Spawns use **tier tokens only** (`opus`/`sonnet`/`haiku`) — resolved to the newest available build at call time, never hardcoded version IDs. `sonnet` resolves to Sonnet 5 (`claude-sonnet-5`). See `plugins/anti-hall/skills/MODEL-POLICY.md`.
