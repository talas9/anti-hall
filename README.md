...
Codex skills include `anti-hall-context-conserve`, `anti-hall-ship-it`, `anti-hall-deadly-loop`, `anti-hall-doctor`, `anti-hall-update`, `anti-hall-jev`, and `anti-hall-omx`. Codex model routing is no longer a pinned slug (OpenAI has silently retired/renamed generations before, most recently `gpt-5.4`/`gpt-5.4-mini`/`gpt-5.3-codex(-spark)`); debate/review gates and settled implementation resolve the **frontier**/**workhorse** category by description text from Codex's own live model cache (`~/.codex/models_cache.json`), and mechanical lookup/commands resolve the **fast** category the same way — see `docs/KB-gpt-5.6.md` and `plugins/anti-hall/companion/lib/codex-models.js`.
...
| `emit-dedupe-reset` | SessionStart (**new in 0.103.0**) | Marks a context loss (startup/resume/`/clear`/compaction) so `lib/emit-dedupe.js`'s UserPromptSubmit suppression re-sends every block the fresh context no longer holds. Fail-open, state-only (no context injected). Registered on both the Claude plugin and the Codex port |
| `task-lifecycle-log` | TaskCreated + TaskCompleted | Log-only: appends one line per task-lifecycle event to the per-session history ledger (`.anti-hall/history/<date>/<session-id>.md`) via the same idempotent index helper `tasklist-guard` uses. No matcher, no evidence gate, never blocks, never injects context. Claude-only — Codex's hook runtime does not expose `TaskCreated`/`TaskCompleted` (see `plugins/anti-hall/codex/README.md`) |
...
| **doctor** | "is anti-hall working?" / "repair anti-hall" / after install/update | confirms Node ≥ 22, all hooks present + syntax-valid, **runs live behavioral self-tests** (spawns real guards with crafted payloads and asserts exit codes), reports context footprint in bytes + estimated tokens. **Env-aware:** also detects + tests OMC, Codex/OMX, and the DevSwarm liveness supervisor (installed-companion state + a per-workspace liveness self-test, `nudged` reads as WARN not FAIL), each silent and skipped when that integration isn't in play. **Repair mode:** plain `doctor` is read-only (no repair pass); `doctor --repair` (alias `--fix`) applies AUTO-SAFE fixes (state migrations — the all-store DevSwarm ones from one registry shared with `update`, stamped done per version so a repeat run skips them — statusline-if-missing, idempotent supervisor/codex refresh) and, only under the DevSwarm gate, GATED daemon fixes (ingest install / wrong-path rebind / stale-script / (v0.58.1) `reconcile` — draining stranded per-worktree queues into the shared store, previously manual-only); `--check` is the read-only CI path, `--dry-run` previews. Every run also reports (never removes) an anti-hall launchd/systemd unit whose working directory is under a temp root or gone, or whose script is gone. **v0.104.0:** `--check` (and repair mode) also runs a read-only `identity-rekey-candidates` report — stores written under a pre-0.104.0 wrong project key, with message counts; nothing is moved automatically |
...
| **jev** | "activate jev" / "enable jev" / "jev status" / "disable jev" / "jev report" | activation front door for the opt-in Jev classifier (`docs/KB-jev-classifier.md`): `scripts/jev-setup.js` reports `status` (enabled?, transport, key present yes/no — never the value, per-integration modes, last-24h call count), `enable`/`disable`, `set-key` (STDIN-only, never argv, 0600 atomic write, prints only `key saved (N chars)`), `test` (one real classification call — ok/latency/confidence or a failure reason, never the key), and `mode <integration> on\|shadow\|off`. Asks which provider a key is for (Vercel AI Gateway, default/live-verified, vs TypeSafe direct, supported but not independently verified) — never guesses from the key's shape. `jev report` (`scripts/jev-report.js`, read-only) gives a per-integration KEEP/REVIEW/REMOVE call. Codex mirror: `anti-hall-jev` |
...
│   ├── skills/                         # root-cause · orchestration · deadly-loop · deadly-loop-multi · ship-it · install-statusline · doctor · system-briefing · update · flutter-debug · activate · simplify · debt · devswarm · handover · defects · jev
