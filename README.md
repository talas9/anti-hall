...
| **defects** (**new in 0.80.0**) | "file an anti-hall bug" / "did they fix my report" / "check my defect reports" | file, list, show, and read rulings on anti-hall's own defect reports via the durable, home-scoped, two-way channel that shipped in v0.78.0 but had no discoverable entry point until now — the `defect-nudge` hook's pointer led nowhere on both ports. `report` accepts `--sym-file`/`--repro-file` so a body doesn't need shell-safe quoting; a report against a defect already ruled `fixed` is derived `regressed` or `staleBuild` from the reporter's installed version. `list --mine` identity no longer depends on the current directory. The `devswarm` skill now points here too. Codex mirror: `anti-hall-defects` |
