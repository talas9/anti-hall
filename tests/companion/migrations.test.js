...
function fakeDw(opts) {
  ...
  return {
    ...
    retireStaleArchivedMarkers: mk('retireStaleArchivedMarkers', (p, e) => ({ ok: true, appDb: true, pending: p, scanned: 1, retired: 0, left: [], errors: e })),
    repairChildSenderLabelsAllStores: mk('repairChildSenderLabelsAllStores', (p, e) => ({ ok: true, pending: p, labels: p, stores: 1, left: [], errors: e })),
    foldReadReceiptsAllStores: mk('foldReadReceiptsAllStores', (p, e) => ({ ok: true, pending: p, ids: p, stores: 1, folded: p, errors: e })),
...
test('registry: every default entry is non-deleting; the deletion-class entry is opt-in only', () => {
  ...
  assert.deepStrictEqual(defaults.map((m) => m.id),
    ['merge-split-backend-stores', 'fold-all-stores', 'heal-orphan-partitions', 'fold-archived-rows', 'fold-archived-family-descriptors', 'repair-reader-floors', 'reconcile-dual-partition-acks', 'mark-app-archived', 'retire-stale-archived-markers', 'repair-child-sender-labels', 'fold-read-receipts']);
...
test('bootstrap: no marker -> one live scan; clean + nothing pending -> marker written; next run skips with no scan', () => {
  ...
  try {
    ...
    assert.strictEqual(dw.calls.length, 11, 'one dry-run scan per default entry');
...
test('pending work: applied once, re-detected, reported fixed, then marked', () => {
  ...
  try {
    ...
    assert.strictEqual(dw.calls.filter((c) => !c.dryRun).length, 11, 'each entry applied exactly once');
