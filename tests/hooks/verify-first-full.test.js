...
const DISCIPLINES = [
  ...
  ['- autonomous-execution:', 'ALWAYS APPLY: autonomous-execution discipline'],
  ['without re-confirming steps that authorization already covers', 'autonomous-execution: no re-confirmation inside an authorized scope'],
  ['ONE consolidated end result', 'autonomous-execution: one consolidated report, not a confirmation stream'],
  ['destructive/irreversible action', 'autonomous-execution: preserves the destructive/irreversible stop-point'],
  ['deletions still require explicit confirmation', 'autonomous-execution: preserves the never-delete-without-confirmation rule'],
  ['DONE still means VERIFIED (Positive Rule 6)', 'autonomous-execution: cross-references Positive Rule 6, does not weaken it'],
  ['EXPANDING scope past what was authorized still needs confirmation', 'autonomous-execution: defers to SCOPE & FIDELITY'],
