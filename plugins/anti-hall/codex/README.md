...
- DevSwarm Phase-1 mechanical-trigger hooks — `hooks/devswarm-parent-inbox.js` / `hooks/devswarm-child-turn.js` (UserPromptSubmit) and `hooks/devswarm-parent-gate.js` / `hooks/devswarm-child-gate.js` (Stop): intentionally **Claude-only, no Codex mirror**, same reasoning as `devswarm-child-role.js` and the liveness supervisor above — all four role-gate on `DEVSWARM_*` env vars (`DEVSWARM_REPO_ID` / `DEVSWARM_SOURCE_BRANCH`) that hivecontrol sets only for the `claude` child sessions it spawns, never for a Codex/OMX session, so registering them would add permanently-inert dead hooks rather than working functionality. Their output contracts (`hookSpecificOutput.additionalContext` on UserPromptSubmit, `{decision:"block"}` on Stop) do match the Codex-registered hooks on those events — the blocker is env-gated inertness, not a payload-shape mismatch. A Codex-side DevSwarm coordination substrate, if ever built, would need its own session/env model and is a separate future effort.
