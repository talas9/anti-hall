...
| auto-archive | Supervisor archives a child in the app only when done, merged, clean, drained, not the Primary, unfocused 10 min and idle (its own mailbox-wake/heartbeat/status pings do not count; real work does); always by explicit workspace id; DevSwarm >= 2.5.3. |
...
- `scripts/settings.js` — `show` [--all] [--section s] [--json] (list), `get <section.key>`, `set <section.key> <value>` (validated), `reset <section.key>` (drop the override), `trust-command-allow [<repo>] --confirmed` (trust a repo's `.anti-hall/command-allow.json` by content hash), `trust-edit-allow [<repo>] --confirmed` (trust a repo's `.anti-hall/edit-allow.json` doc-edit globs by content hash).
...
- `scripts/devswarm.js` (`help <verb>` for detail) — mailbox: `send` (post to --to/--to-primary/--broadcast; `--quiet` for a one-line "sent seq N -> X, B bytes, ok"/loud "ok:false ..." instead of raw JSON; `--to <sibling> --cc-primary` also copies the Primary with the identical body, best-effort, reported under `ccPrimary`), `relay <seq|receipt> --to <id> [--note-file f]` (forward a message THIS caller already received, verbatim, with a provenance header — fails loud on a byte-length mismatch or an empty source body), `inbox` (count/read/pull/read-primary/ack-primary/peek-primary; `read-primary --format text` prints from/seq/body plain text instead of JSON, two-step ack default unchanged; opt-in `--ack-after-print` acks right after printing), `roster` (who is on the mesh), `mesh` (read the mesh), `heartbeat` (liveness + summary), `wake-directive` (reprint the wake instructions); registry: `register`, `ensure`, `register-primary` (Primary checkout only), `primary` (`status` shows the Primary seat; `takeover` demotes a conflicting session), `workspaces`, `migrate`, `migrate-owner-keys`, `reconcile`, `reconcile-registry`, `reconcile-active`, `reap-stale`, `reap-orphans`; lifecycle: `spawn` (create a workspace — raw argv tail forwarded to `hivecontrol workspace create`: `-s/--source`, `-a/--agent`, `-p/--prompt`, `-r/--remote`, `-t/--title`, plus anti-hall's own `--from-local`; `help spawn` has the full detail), `merge`, `ready-check <sha>` (read-only verdict on a child's READY claim: fast-forward, submodule-pointer changes, watched-dir deletions, files outside an allowlist; never merges), `gate` (finish gates), `gate-intent` (state why a Stop block is intentional), `nudge`, `archive`, `unarchive`, `archive-ignore`, `archive-unignore`, `archive-request` (ask a child to archive itself), `auto-archive` (show the plan), `prune-archived` (owner-approved delete), `retention` (status/run/restore); app DB: `app-state`, `app-sync`, `sync-ui` (screenshot sync); health: `diagnose`, `healthcheck`, `logs`; `skip <guard> [--ttl m]` (user-requested skip only).
...
- **Auto Handover**: `autoHandover.enabled`=true, `autoHandover.pct`=85, `autoHandover.maxTokens`=0, `autoHandover.nag`=true, `autoHandover.nagStepPct`=5, `autoHandover.nagQuietMin`=15, `autoHandover.gateNewWork`=true, `autoHandover.gateBudgetPct`=5, `autoHandover.decisivePrompt`=true
- **Guards**: `guards.mergeGate`=false, `guards.shipitGate`=false, `guards.outputVerifyGuard`=true, `guards.failureRootCauseNudge`=true, `guards.repoSelfDrift`=true, `guards.stashGuard`=false (safety: --confirmed), `guards.emitDedupe`=true, `guards.editGuardAllow`=—* (safety: --confirmed), `guards.allowSubagentMailbox`=false* (safety: --confirmed), `guards.reaperMatch`=—*, `guards.reaperExclude`=—*, `guards.tasklistWorkThreshold`=3*, `guards.progressFreshMs`=1800000*, `guards.apiGuardThirdparty`=false*, `guards.modelRouting`=strict, `guards.modelRoutingDeployFloor`=sonnet, `guards.apiGuard`=true, `guards.speculationGuard`=true, `guards.claimLedger`=true, `guards.taskGuard`=true, `guards.tasklistGuard`=true, `guards.scanThrottle`=true, `guards.silentAgentNudge`=true, `guards.silentAgentNudgeMin`=20*, `guards.reaperCodexBroker`=true*, `guards.reaperCodexBrokerMinAgeS`=1800*, `guards.injectionRepeatEvery`=10*, `guards.codexQuotaDetect`=true, `guards.allowReadOnlyVerify`=true, `guards.allowReadOnlyVerifyScripts`=true, `guards.stopHookVersionDowngrade`=true*, `guards.stopAck`=true*, `guards.projectCommandAllow`=true, `guards.projectEditAllow`=true, `guards.allowPlainPush`=true, `guards.allowGcloudReads`=true, `guards.allowBackgroundScratchScripts`=true, `guards.taskGuardOwnerBlockedMarker`=true*
...
- **Updates / Maintenance**: `updates.quiet`=false, `updates.allowCachePrune`=true, `updates.reconcileBudgetMs`=60000*, `updates.postpullBudgetMs`=90000*, `updates.sweepBudgetMs`=20000*
...
- **Jev integration**: `jevIntegrations.speculation`=on, `jevIntegrations.triage`=on, `jevIntegrations.newRequest`=shadow, `jevIntegrations.claimLedger`=shadow, `jevIntegrations.outputVerifyGuard`=shadow, `jevIntegrations.gitGuardSelfCredit`=shadow, `jevIntegrations.modelRouting`=shadow, `jevIntegrations.tasklistTrivial`=shadow, `jevIntegrations.codexNudgeSubstantial`=shadow, `jevIntegrations.mergeGateHedge`=shadow, `jevIntegrations.parentGateQuestion`=shadow, `jevIntegrations.supervisorBlockerLabel`=shadow, `jevIntegrations.findingDedup`=on, `jevIntegrations.postHandoverGate`=off
- **DevSwarm**: `devswarm.hivecontrol`=—, `devswarm.supervisorMode`=auto, `devswarm.requiredGates`=done,merged,tests_passed, `devswarm.inboxCmd`=—, `devswarm.heldPartitions`=—*, `devswarm.childGateStrict`=true*, `devswarm.parentGateCap`=3*, `devswarm.parentGateNeglectMinUnread`=0*, `devswarm.parentGateBusyFreshMin`=5*, `devswarm.parentGateBusyMaxAgeMin`=60*, `devswarm.parentGateNeglectGraceMin`=1*, `devswarm.activeFloorPct`=50*, `devswarm.archivedCacheMaxAgeMs`=—*, `devswarm.archivedGraceMs`=600000*, `devswarm.cooldownSec`=600*, `devswarm.idleSec`=900*, `devswarm.dormantMs`=1800000*, `devswarm.drainTtlMs`=600000*, `devswarm.graceSec`=5*, `devswarm.maxRecoveries`=3*, `devswarm.intervalSec`=90*, `devswarm.migrateMarkRead`=false*, `devswarm.monitorTimeoutSec`=30*, `devswarm.monitorNoOkFailMin`=10*, `devswarm.nudgeCooldownSec`=120*, `devswarm.nudgeMaxAttempts`=2*, `devswarm.nudgeWindowSec`=180*, `devswarm.postSpawnGraceSec`=120*, `devswarm.reapedRetentionDays`=30*, `devswarm.receiptWindowMs`=300000*, `devswarm.archiveRequestRenagHours`=24*, `devswarm.reconcileSweep`=auto*, `devswarm.reconcileSweepSec`=900*, `devswarm.rowStaleMs`=86400000*, `devswarm.sendReceiptRetentionDays`=7*, `devswarm.summaryRetentionDays`=30*, `devswarm.wakeCron`=7,37 * * * **, `devswarm.wakeWatchPollMs`=2000*, `devswarm.childGateRetentionDays`=14*, `devswarm.housekeepingSweep`=auto*, `devswarm.housekeepingSweepSec`=3600*, `devswarm.supervisorLogRotateBytes`=10485760*, `devswarm.inboxGraceSec`=120*, `devswarm.supervisorSweepBudgetMs`=20000*, `devswarm.supervisorBlockerLabelReaskSec`=21600*, `devswarm.autoArchive.mode`=on, `devswarm.autoArchive.idleMin`=30*, `devswarm.autoArchive.maxPerSweep`=3*, `devswarm.autoArchive.ignorePings`=true*, `devswarm.retention.days`=30*, `devswarm.retention.maxStoreMB`=100*, `devswarm.retention.keepPerPartition`=200*, `devswarm.retention.archive`=true*, `devswarm.retention.archiveMaxMB`=0*, `devswarm.parentGate`=true, `devswarm.childGate`=true, `devswarm.parentInbox`=true, `devswarm.childTurn`=true, `devswarm.childRole`=true, `devswarm.childDrain`=true, `devswarm.parentReplyTracker`=true, `devswarm.commsGuard`=true, `devswarm.inboxReadGuard`=true, `devswarm.wakeWatch`=true, `devswarm.appSync`=true, `devswarm.screenshotSync`=true, `devswarm.spawnFromOrigin`=true, `devswarm.sendMultiRecipient`=true, `devswarm.spawnStrictFlagValues`=true, `devswarm.spawnFetchTtlSec`=300*, `devswarm.spawnCreateTimeoutMs`=180000*, `devswarm.stableLauncher`=true*
...
Env-only: `ANTIHALL_CONTEXT_WINDOW_TOKENS`, `ANTI_HALL_THROTTLE_PATTERNS`, `ANTIHALL_DEVSWARM_APP_DB` (path or `off`). "(safety: --confirmed)" = a safety key: `set` to the risky value, and a `reset` whose fallback value is risky (e.g. resetting an armed `guards.stashGuard`, default off), need `--confirmed` (a human direct command, or the agent asking the user and getting a yes) — without it nothing changes and a one-line factual warning is returned instead; re-arming a guard never needs it; a direct user ask to change it IS the confirmation. A settings.json value counts like any other (normal precedence); nothing mechanically stops an agent writing that file directly — the owner chose consent over an extra guard, so never hand-edit a safety key there to get around the confirmation. No switch on purpose: skip-guard, coordinator-detect, omc-detect, phase-tracker, fable-availability, codex-availability, emit-dedupe-reset, agent-watchdog, command-guard's data-safety sub-guards (`settings.js show` gives the reasons).
