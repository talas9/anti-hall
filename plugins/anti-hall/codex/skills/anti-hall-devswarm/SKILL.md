...
| `roster [--ack]` | none | This project's registry + `working_on` + a `recent[]` broadcast digest, folded with a read-only native-children view. Every row prints BOTH `id` and `meshId` — either one addresses it via `send --to`. A row whose Claude transcript is paused on an unresolved `AskUserQuestion`/`ExitPlanMode` gets a `waiting-on-human` hint (same `childBusyState` detector the parent gate reuses) — a Codex workspace has no such transcript shape to classify, so this hint simply never fires for a Codex row, harmlessly (`known:false`, same fail-open posture as every other liveness signal here). Pure `computeSummary` read (always LIVE — see the edge-case table below for what that means when the ingest daemon is stale). | Get the current mesh state / who's doing what, and the addressing fields to copy into `send --to`. | **Read-only** (plain); **writes** the broadcast cursor with `--ack`. |
| `mesh read [--peek] [--seq N]` | none | Alias of `roster --ack`. `--peek` reads without advancing the cursor; `--seq N` reads from an explicit historical seq instead of the caller's cursor and always implies peek. Neither flag changes the default (no-flag) behavior. | Same as `roster --ack`, named for discovery; `--peek`/`--seq` for re-inspecting history without consuming it. | **Writes** unless `--peek`/`--seq`, then **read-only.** |
