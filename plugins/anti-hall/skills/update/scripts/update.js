...
function wakeMonitorPostUpdate(opts) {
  ...
  try {
    ...
    const armCmd = 'call the `Monitor` tool with command `node ' + watcherPath + '`, arming it with persistent: true if your Monitor tool supports that field — otherwise set timeout_ms to its maximum and re-arm when the tool emits its final/expired event (never run two watchers; the watcher\'s own lock also guards this, and the cron job stays the fallback either way) (or run that command yourself in a background terminal)';
    if (!identity) {
      return {
