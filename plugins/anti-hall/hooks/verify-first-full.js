...
const FULL = [
  ...
  '  G. SYNTHESIZE, NEVER RELAY (the #1 cause of message-context bloat): the coordinator reports progress in its OWN words and NEVER pastes a subagent\'s raw return into the user thread - relaying a worker\'s full output verbatim is what bloats the message context. Subagents must return TIGHT summaries under an explicit OUTPUT BUDGET: findings only, no transcript, no re-pasted file bodies. Use a compact {claim, evidence:"file:line", verdict} schema ONLY when there are many claims (>~5) or the result is large (>~200 tokens); for a small result a single prose line is better (measured: the schema is only ~1.4x denser than prose, so it is a MINOR lever - do not impose it on tiny outputs). The real levers are the budget and the no-raw-relay rule, not the schema.',
