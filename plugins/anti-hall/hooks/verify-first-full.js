...
const FULL = [
  ...
  '  A. COMMAND DELEGATION (TOP RULE): NEVER run verbose/long/state-changing commands (build, test, deploy, push, pull, install, migrate, dumps, bulk scripts) OR broad reads/Grep/Glob/Bash code-nav searches (git grep, find, rg, ag, multi-file sweeps) inline. ALWAYS delegate them to a subagent that returns only a tight summary. Bloating the orchestrator context with raw output DEGRADES MODEL QUALITY AND INDUCES HALLUCINATION - the exact failure this plugin prevents.',
  ...
  '  E. Handle INLINE only genuinely atomic things: a direct answer, a single known-file read at a known path, and synthesis/decisions the coordinator must do itself. If a quick inline task balloons, delegate immediately.',
  '  E2. GRAPHIFY-FIRST: when a graphify graph exists (graphify-out/ or .planning/graphs/), ensure it is fresh (/graphify --obsidian) then query it (/graphify query "...") BEFORE any Grep/Glob/raw code search and BEFORE feature-launch analysis.',
  ...
  '  - orchestration discipline: the ORCHESTRATION DISCIPLINE block above. Command delegation is the top rule (never inline heavy commands, broad reads, or code-nav searches - delegate to subagent; bloating context induces hallucination). Non-blocking main thread; priority-sorted task list; actively drain tasks; bias toward delegating any tool/file/command/build/test/search work to subagents (inline only genuinely atomic things); parallel agents when independent; graphify-first when a graph exists.',
