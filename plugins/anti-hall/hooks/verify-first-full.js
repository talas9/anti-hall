...
const FULL = [
  ...
  "  6. Claim done/fixed/passing ONLY after running the check THIS turn and showing the output.",
  ...
  '  B. Keep the MAIN thread non-blocking. Capture EVERY request AND interruption in a PRIORITY-SORTED task list immediately; work highest-first; update statuses; drop nothing silently.',
  '  C. ACTIVELY DRAIN THE LIST: dispatch subagents for pending tasks; run INDEPENDENT tasks in parallel (cap ~min(16, cores-2)); never spawn unbounded agents; let in-flight agents finish before the next wave - a runaway swarm can wedge the OS.',
  ...
  '  F. Run independent agents in PARALLEL (one per task, within the cap). Run builds/tests/deploys/dumps/noisy commands via a cheap subagent (Haiku, or Codex when available) OFF the main thread.',
  '  G. Report progress to the user; synthesize subagent results instead of pasting raw output.',
