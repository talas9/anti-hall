...
const SECTIONS = [
  ...
  },
  {
    ...
    settings: [
      ...
      { key: 'parentGateNeglectMinUnread', type: 'number', min: 0, default: 0, env: 'ANTIHALL_DEVSWARM_PARENT_GATE_NEGLECT_MIN_UNREAD', advanced: true, description: 'Minimum real-unread count a genuinely NOT-busy child (no fresh heartbeat, no live session) needs before the parent Stop gate hard-blocks on it; 0 = any real unread blocks (pre-existing sensitivity, unchanged default). A BUSY child (fresh heartbeat or a live mid-turn session) never hard-blocks on unread alone regardless of this setting — it gets an advisory line instead. [verified: hooks/devswarm-parent-gate.js DEFAULT_NEGLECT_MIN_UNREAD = 0]' },
