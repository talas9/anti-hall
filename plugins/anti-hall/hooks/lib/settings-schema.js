...
const SECTIONS = [
  ...
  },
  {
    ...
    settings: [
      ...
      { key: 'reaperCodexBroker', type: 'boolean', default: true, env: 'ANTIHALL_REAPER_CODEX_BROKER', advanced: true, description: 'companion/mcp-reaper.js: also reap orphaned openai-codex plugin app-server-broker.mjs helper processes (PPID 1, or a dead/reaper parent; exact script-name match under a codex plugin path) — same leak shape as the generic MCP signature (no PR_SET_PDEATHSIG on macOS pins archived DevSwarm worktrees) but a distinct non-MCP-protocol broker, matched separately so this never loosens the MCP matcher itself. Only takes effect when the opt-in companion reaper is installed and running. [verified: companion/mcp-reaper.js — default on]' },
      { key: 'reaperCodexBrokerMinAgeS', type: 'number', min: 0, default: 60, env: 'ANTIHALL_REAPER_CODEX_BROKER_MIN_AGE_S', advanced: true, description: 'Minimum age in seconds an orphaned app-server-broker.mjs must have before the reaper class above will select it (same default as the SessionEnd reaper\'s own orphan-age gate); an unresolvable age is always skipped, never reaped. [verified: companion/mcp-reaper.js — default 60s]' },
