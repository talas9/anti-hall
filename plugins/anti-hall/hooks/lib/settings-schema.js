...
const SECTIONS = [
  ...
  },
  {
    ...
    settings: [
      ...
      { key: 'spawnStrictFlagValues', type: 'boolean', default: true, env: 'ANTIHALL_DEVSWARM_SPAWN_STRICT_FLAG_VALUES', pluginOption: 'devswarm_spawn_strict_flag_values', description: '`devswarm.js spawn`: refuse when a value-taking create option (-s/--source, -a/--agent, -t/--title, -p/--prompt) has no value or is handed what looks like the next option (e.g. `-t -p "brief"` would make "-p" the title), naming the flag. [verified: scripts/devswarm.js spawnFlagValueError — default on]' },
      { key: 'sendMultiRecipient', type: 'boolean', default: true, env: 'ANTIHALL_DEVSWARM_SEND_MULTI_RECIPIENT', pluginOption: 'devswarm_send_multi_recipient', description: '`devswarm.js send`: `--to <id1>,<id2>` or a repeated `--to` sends the same body to each (deduped) recipient, attempts every one even after a failure, and exits non-zero if any failed. false restores the old parsing (last --to wins). [verified: scripts/devswarm.js cmdSendMulti — default on]' },
      // ---- 0.109.0: spawn speed ----
