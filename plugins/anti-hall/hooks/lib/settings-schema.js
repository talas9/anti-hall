...
const SECTIONS = [
  ...
  },
  {
    ...
    settings: [
      ...
      { key: 'reaperCodexBroker', type: 'boolean', default: true, env: 'ANTIHALL_REAPER_CODEX_BROKER', advanced: true, description: 'companion/mcp-reaper.js: also reap abandoned openai-codex plugin app-server-broker.mjs helper processes. It is spawned detached+unref ON PURPOSE (PPID 1 is normal for a LIVE broker, not evidence of death), so this is NOT the generic MCP parent-death invariant: it is reaped only when its --cwd directory no longer exists, or no live claude/codex process has a cwd equal to/an ancestor of/a descendant of it (a session at a workspace root still owns a broker --cwd inside a submodule under it), AND it is older than guards.reaperCodexBrokerMinAgeS. Matched by an exact script-name + codex-plugin-path signature, kept fully separate so this never loosens the MCP matcher itself. Only takes effect when the opt-in companion reaper is installed and running. [verified: companion/mcp-reaper.js — default on]' },
      { key: 'reaperCodexBrokerMinAgeS', type: 'number', min: 0, default: 1800, env: 'ANTIHALL_REAPER_CODEX_BROKER_MIN_AGE_S', advanced: true, description: 'Minimum age in seconds an app-server-broker.mjs whose owner cannot be found must have before the reaper class above will select it (30 minutes by default — deliberately conservative since PPID gives no death signal for this class); an unresolvable age is always skipped, never reaped. [verified: companion/mcp-reaper.js — default 1800s]' },
  ...
  },
  {
    ...
    settings: [
      ...
      { key: 'postHandoverGate', type: 'enum', values: ['on', 'shadow', 'off'], default: 'shadow', pluginOption: 'jev_integration_post_handover_gate', description: 'Does this new request fit in the remaining post-handover context budget (advisory trust; askDetached fire-and-forget, zero latency). Shadow-only today: logged for `jev report`, never changes the gate directive. [read by: hooks/lib/jev-assist.js getMode, hooks/auto-handover.js]' },
    ],
  },
  {
    key: 'devswarm',
