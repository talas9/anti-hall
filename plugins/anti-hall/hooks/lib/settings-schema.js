...
const SECTIONS = [
  ...
  },
  {
    ...
    settings: [
      ...
      { key: 'allowReadOnlyVerifyScripts', type: 'boolean', default: true, env: 'ANTIHALL_ALLOW_READ_ONLY_VERIFY_SCRIPTS', pluginOption: 'guards_allow_read_only_verify_scripts', description: 'command-guard read-only verify, script form: lets the coordinator run `<python*|node|ruby|perl|php> <existing script file> --check|--dry-run|--list` piped to a bounded sink inline. No inline-code flag (-c/-e/-m/--eval), no stdin/heredoc script, no wrapper verb, and the remaining arguments must be non-heavy. Requires guards.allowReadOnlyVerify. [verified: hooks/command-guard.js isInterpreterScriptCheck — default on]' },
      ...
      { key: 'projectEditAllow', type: 'boolean', default: true, env: 'ANTIHALL_PROJECT_EDIT_ALLOW', pluginOption: 'guards_project_edit_allow', description: 'edit-guard per-project doc-edit allowlist: a repo may list repo-relative globs in <repo-toplevel>/.anti-hall/edit-allow.json ({"paths":["docs/**"]}) that the MAIN THREAD may Edit/Write directly. Applies only after the user trusts that exact file content (settings.js trust-edit-allow <repo> --confirmed); an edit revokes trust. Never matches outside the repo, .git, .anti-hall, .claude, .codex, hooks config or ~/.claude. Kill-switch: false disables it. [verified: hooks/edit-guard.js isProjectEditAllowed — default on]' },
      { key: 'allowPlainPush', type: 'boolean', default: true, env: 'ANTIHALL_ALLOW_PLAIN_PUSH', pluginOption: 'guards_allow_plain_push', description: 'command-guard "allow plain push": in the MAIN THREAD ONLY, lets `git add`/`git commit`/a plain `git push [-q|--quiet] [remote] [ref]` (ref omitted, HEAD, or the current branch only), and &&/; chains made up only of those three, run inline instead of being delegated. Also allows ONE optional leading `cd <path>` (only when it realpaths to the payload cwd\'s own repo toplevel or a directory inside it) and optional trailing read-only segments (`git log --oneline [-N]`, `git status [--short|-s]`, `git show --stat [-N|HEAD]`) ONLY after a push segment. --force/-f/--force-with-lease/--force-if-includes/--mirror/--delete/-d/--all/--tags/+refspec/src:dst-to-another-branch/any other flag combined with -q stay exactly as blocked as before; git-guard.js keeps its own independent force-push/AI-credit checks. [verified: hooks/command-guard.js isAllowedPlainPushChain — default on]' },
      { key: 'allowGcloudReads', type: 'boolean', default: true, env: 'ANTIHALL_ALLOW_GCLOUD_READS', pluginOption: 'guards_allow_gcloud_reads', description: 'command-guard narrow read-only gcloud: in the MAIN THREAD ONLY, lets `gcloud auth print-access-token`, `gcloud <group…> <describe|list|get-iam-policy|read> … --format=json|yaml|value(...)` (optionally piped into a bounded sink or jq), and `T=$(gcloud auth print-access-token); curl -s|-sS [-H "Authorization: Bearer $T"] <https URL>` (host must be googleapis.com or a subdomain — no userinfo, IP literal, -L, --resolve, --connect-to, -x/--proxy, --url or -K/--config; GET only, piped into a bounded sink/jq or capped with --max-filesize) run inline. Mutating gcloud verbs (create/delete/deploy/set/update/add-iam-policy-binding/remove-*/patch/import/export/rollback/start/stop/ssh/scp/submit/run/apply), curl -X other than GET, -d/--data*/-F/-T/--upload-file/-o/-O/--output, @file and chained segments stay blocked. [verified: hooks/command-guard.js isAllowedGcloudReadCommand — default on]' },
      { key: 'allowBackgroundScratchScripts', type: 'boolean', default: true, env: 'ANTIHALL_ALLOW_BACKGROUND_SCRATCH_SCRIPTS', pluginOption: 'guards_allow_background_scratch_scripts', description: 'command-guard background scratch scripts: in the MAIN THREAD ONLY, a Bash call with run_in_background: true may run ONE segment `<python3|node|sh|bash> <script file> [args…]` when the file is an existing regular file inside the session scratchpad or a tmp root (os.tmpdir(), /tmp, /private/tmp; realpath-checked). No interpreter option before the file (-c/-e/…), no env prefix or wrapper, no chaining/pipes, no substitution or expansion, no stdin redirect, no write redirect outside the scratchpad/tmp. Foreground runs keep the normal rules. [verified: hooks/command-guard.js isBackgroundScratchScript — default on]' },
      ...
      { key: 'taskGuardOwnerBlockedMarker', type: 'boolean', default: true, env: 'ANTIHALL_TASK_GUARD_OWNER_BLOCKED_MARKER', pluginOption: 'guards_task_guard_owner_blocked_marker', advanced: true, description: 'task-guard IDLE NEGLECT: honor an explicit owner-blocked marker (metadata.blockedOn / blockedOn === \'owner\'|\'user\'|\'human\', or an "OWNER:" / "OWNER DECISION" subject prefix) as non-dispatchable, instead of requiring a fake blockedBy dependency to silence the nag. Off reverts to pre-marker behavior (only a real blockedBy id suppresses idle-neglect). [verified: hooks/task-guard.js isOwnerBlocked]' },
  ...
  },
  {
    ...
    settings: [
      ...
      { key: 'wakeCron', type: 'string', default: '7,37 * * * *', env: 'ANTIHALL_DEVSWARM_WAKE_CRON', advanced: true, description: 'Wake-poll cron schedule override (treated as untrusted input). [verified: hooks/lib/devswarm-wake.js:75 WAKE_CRON_DEFAULT = \'7,37 * * * *\']' },
      ...
      { key: 'spawnStrictFlagValues', type: 'boolean', default: true, env: 'ANTIHALL_DEVSWARM_SPAWN_STRICT_FLAG_VALUES', pluginOption: 'devswarm_spawn_strict_flag_values', advanced: true, description: '`devswarm.js spawn`: refuse when a value-taking create option (-s/--source, -a/--agent, -t/--title, -p/--prompt) has no value or is handed what looks like the next option (e.g. `-t -p "brief"` would make "-p" the title), naming the flag. [verified: scripts/devswarm.js spawnFlagValueError — default on]' },
      { key: 'sendMultiRecipient', type: 'boolean', default: true, env: 'ANTIHALL_DEVSWARM_SEND_MULTI_RECIPIENT', pluginOption: 'devswarm_send_multi_recipient', advanced: true, description: '`devswarm.js send`: `--to <id1>,<id2>` or a repeated `--to` sends the same body to each (deduped) recipient, attempts every one even after a failure, and exits non-zero if any failed. false restores the old parsing (last --to wins). [verified: scripts/devswarm.js cmdSendMulti — default on]' },
      // ---- 0.109.0: spawn speed ----
